...
class ProviderReviewsViewModel extends ChangeNotifier {
  ...
  ProviderReviewsViewModel({required ApiClient apiClient, required this.providerId})
  ...
  Future<void> loadProviderReviews({bool reset = true}) async {
    if (reset) {
      _currentPage = 1;
      _reviews.clear();
    }
    ...
    try {
      final response = await _reviewRepository.getProviderReviews(
        providerId,
      ...
      if (response.data != null) {
        ...
        _error = response.message ?? 'حدث خطأ في جلب مراجعات البائع';
      }
    } catch (e) {
      _error = 'حدث خطأ في جلب مراجعات البائع';
      if (kDebugMode) {
  ...
  Future<void> loadMoreReviews() async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('Error loading more provider reviews: $e');
      }
    } finally {
  ...
  Future<void> refreshReviews() async {
    await loadProviderReviews(reset: true);
  }
